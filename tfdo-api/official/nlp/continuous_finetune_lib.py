...
def run_continuous_finetune(
  ...
  Args:
    mode: A 'str', specifying the mode. continuous_train_and_eval - monitors a
      ...
      checkpoint, finetune the export by training it (probably on another dataset
      or with another task), then evaluate the finetuned export.
    params: ExperimentConfig instance.
    model_dir: A 'str', a path to store export checkpoints and summaries.
  ...
  # can have significant impact on export speeds by utilizing float16 in case of
  ...
  for pretrain_ckpt in tf.train.checkpoints_iterator(
    ...
    # objects.
    # TODO(b/169178664): Fix cycle reference in Keras export and revisit to see
    # if we need gc here.
    gc.collect()
