...
class ElectraPretrainer(tf.keras.Model):
  """ELECTRA network training export.
  ...
  export (at generator side) and classification networks (at discriminator side)
  that are used to create the training objectives.

  *Note* that the export is constructed by Keras Subclass API, where layers are
  ...
  def call(self, inputs):
    ...
    Args:
      inputs: A dict of all inputs, same as the standard BERT export.

    Returns:
      outputs: A dict of pretrainer export outputs, including
