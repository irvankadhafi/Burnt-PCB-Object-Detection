...
class BertPretrainer(tf.keras.Model):
  ...
  instantiates the masked language export and classification networks that are
  used to create the training objectives.

  *Note* that the export is constructed by
  ...
  def __init__(self,
    ...
    # constructor, resulting in this object having all the properties of a export
    ...
    # compatibility with an earlier version of this export which did not track
...
class BertPretrainerV2(tf.keras.Model):
  """BERT pretraining export V2.

  Adds the masked language export head and optional classification heads upon the
  transformer encoder.
