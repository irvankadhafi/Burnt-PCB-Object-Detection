...
class DualEncoderTest(keras_parameterized.TestCase):
  ...
  def test_dual_encoder(self, hidden_size, output):
    ...
    # Build a transformer network to use within the dual encoder export.
    vocab_size = 100
    sequence_length = 512
    ...
    # Create a dual encoder export with the created network.
  ...
  def test_dual_encoder_tensor_call(self, hidden_size, output):
    ...
    # Build a transformer network to use within the dual encoder export. (Here,
    ...
    # Create a set of 2-dimensional data tensors to feed into the export.
    word_ids = tf.constant([[1, 1], [2, 2]], dtype=tf.int32)
    mask = tf.constant([[1, 1], [1, 0]], dtype=tf.int32)
    type_ids = tf.constant([[1, 1], [2, 2]], dtype=tf.int32)

    # Invoke the export export on the tensors. In Eager mode, this does the
  ...
  def test_serialize_deserialize(self):
    """Validate that the dual encoder export can be serialized / deserialized."""
    # Build a transformer network to use within the dual encoder export. (Here,
    # we use a short sequence_length for convenience.)
    sequence_length = 32
    test_network = networks.BertEncoder(
    ...
    # Create a dual encoder export with the created network. (Note that all the
    ...
    # Create another dual encoder export via serialization and deserialization.
