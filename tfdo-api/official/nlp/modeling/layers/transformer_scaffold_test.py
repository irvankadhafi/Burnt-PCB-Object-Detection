...
class TransformerLayerTest(keras_parameterized.TestCase):
  ...
  def test_layer_invocation(self):
    ...
    # Create a export from the test layer.
    model = tf.keras.Model(data_tensor, output_tensor)

    # Invoke the export on test data. We can't validate the output data itself
  ...
  def test_layer_restoration_from_config(self):
    ...
    # Serialize the export config. Pass the serialized data through json to
    # ensure that we can serialize this layer to disk.
    serialized_data = model.get_config()

    # Create a new export from the old config, and copy the weights. These models
