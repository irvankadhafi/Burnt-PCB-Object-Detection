...
class BertTokenizer(tf.keras.layers.Layer):
  ...
  def __init__(self, *,
    ...
    # (e.g., the final export built using TF Hub) does not properly track
  ...
  def __init__(self,
    ...
    Args:
      lower_case: A Python boolean indicating whether to lowercase the string
        ...
        (case folding) normalization into the Sentencepiece export itself and
        avoid this extra step.
      model_file_path: A Python string with the path of the sentencepiece export.
        Exactly one of `model_file_path` and `model_serialized_proto` can be
        specified. In either case, the Keras export config for this layer will
        store the actual proto (not a filename passed here).
      model_serialized_proto: The sentencepiece export serialized proto string.
      tokenize_with_offsets: A Python boolean. If true, this layer calls
      ...
      strip_diacritics: Whether to strip diacritics or not. Note that stripping
        ...
        into custom normalization rules for the Sentencepiece export itself to
    ...
    with tf.init_scope():
      ...
      for name, token_id, inverse_token in zip(special_tokens,
        ...
        if special_tokens[name] == inverse_token:
          ...
          logging.warning(
              "Could not find %s as token \"%s\" in sentencepiece export, "
              "got \"%s\"", name, special_tokens[name], inverse_token)
    return result


class BertPackInputs(tf.keras.layers.Layer):
  """Packs tokens into export inputs for BERT."""
  ...
  def __init__(self,
    ...
    Args:
      seq_length: The desired output length. Must not exceed the max_seq_length
        that was fixed at training time for the BERT export receiving the inputs.
