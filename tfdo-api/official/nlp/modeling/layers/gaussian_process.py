...
class RandomFeatureGaussianProcess(tf.keras.layers.Layer):
  ...
  During training, the export updates the maximum a posteriori (MAP) logits
  estimates and posterior precision matrix using minibatch statistics. During
  inference, the export divides the MAP logit estimates by the predictive
  ...
  def reset_covariance_matrix(self):
    ...
    This function is useful for reseting the export's covariance matrix at the
...
class LaplaceRandomFeatureCovariance(tf.keras.layers.Layer):
  ...
  export features in minibatches.
  ...
  def call(self, inputs, logits=None, training=None):
    ...
    Args:
      inputs: (tf.Tensor) GP random features, shape (batch_size,
        gp_hidden_size).
      logits: (tf.Tensor) Pre-activation output from the export. Needed
...
def mean_field_logits(logits, covariance_matrix=None, mean_field_factor=1.):
  """Adjust the export logits so its softmax approximates the posterior mean [1].
