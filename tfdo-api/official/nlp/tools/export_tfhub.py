...
flags.DEFINE_string(
    ...
    "Needed for --export_type export and model_with_mlm.")
flags.DEFINE_string(
...
flags.DEFINE_bool(
    ...
    "When the export is trained using `BertPretrainerV2`, the pool layer "
...
flags.DEFINE_string(
    ...
    "File path to a pre-trained export checkpoint. "
    "For --export_type export, this has to be an object-based (TF2) checkpoint "
    "that can be restored to `tf.train.Checkpoint(encoder=encoder)` "
    "for the `encoder` defined by the config file."
    "(Legacy checkpoints with `export=` instead of `encoder=` are also "
    "supported for now.) "
...
flags.DEFINE_string(
    ...
    "the vocabulary file that the encoder export was trained with. "
    "Exactly one of --vocab_file and --sp_model_file can be set. "
    "Needed for --export_type export, model_with_mlm and preprocessing.")
flags.DEFINE_string(
    "sp_model_file", None,
    "For encoders trained on SentencepieceTokenzier input: "
    "the SentencePiece .export file that the encoder export was trained with. "
    "Exactly one of --vocab_file and --sp_model_file can be set. "
    "Needed for --export_type export, model_with_mlm and preprocessing.")
flags.DEFINE_bool(
    "do_lower_case", None,
...
flags.DEFINE_bool(  # TODO(b/175369555): Remove this flag and its use.
    ...
    "Export a preprocessing export without tf.Assert ops. "
...
def main(argv):
  ...
  if FLAGS.export_type in ("export", "model_with_mlm"):
