...
def _create_model(
  ...
  """Creates the export to export and the export to restore the checkpoint.
  ...
  Returns:
    A Tuple of (1) a Keras export that will be exported, (2) a `BertPretrainerV2`
    object or `BertEncoder` object depending on the value of `with_mlm`
    argument, which contains the first export and will be used for restoring
  ...
  if with_mlm:
    ...
    # from `mlm` sub-object will not be included in the core export.
...
def export_model(export_path: Text,
  ...
  Args:
    ...
    sp_model_file: The path to the sentencepiece export file, or None.
  ...
  # full vocab, not the high-level operations from the preprocessing export.
  if vocab_file:
...
def create_preprocessing(*,
  ...
  if vocab_file:
    ...
    tokenize = layers.SentencepieceTokenizer(
        ...
        lower_case=do_lower_case,
        strip_diacritics=True,  #  Strip diacritics to follow ALBERT export.
        tokenize_with_offsets=tokenize_with_offsets)

  # The root object of the preprocessing export can be called to do
...
# replaces them by a no-op while building the exported export. This is fragile,
# so _check_no_assert() validates the result. The resulting export should be fine
