...
"""Train and evaluate the Transformer export.
...
def translate_and_compute_bleu(model,
  ...
  Args:
    model: A Keras export, used to generate the translations.
...
class TransformerTask(object):
  """Main entry of Transformer export."""
  ...
  def train(self):
    ...
    while current_step < flags_obj.train_steps:
      ...
      if params["use_ctl"]:
        ...
        if self.use_tpu:
          raise NotImplementedError(
              "Keras export.fit on TPUs is not implemented.")
        history = model.fit(
            train_ds,
  ...
  def eval(self):
    """Evaluates the export."""
    distribution_strategy = self.distribution_strategy if self.use_tpu else None

    # We only want to create the export under DS scope for TPU case.
  ...
  def predict(self):
    """Predicts result from the export."""
    params = self.params
    flags_obj = self.flags_obj
  ...
  def _load_weights_if_possible(self, model, init_weight_path=None):
    """Loads export weights when it is provided."""
    if init_weight_path:
...
def main(_):
  ...
  # Execute flag override logic for better export performance
