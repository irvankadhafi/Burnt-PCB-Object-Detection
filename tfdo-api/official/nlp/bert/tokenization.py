...
def validate_case_matches_checkpoint(do_lower_case, init_checkpoint):
  ...
  if is_bad_config:
    raise ValueError(
        ...
        "However, `%s` seems to be a %s export, so you "
        "should pass in `--do_lower_case=%s` so that the fine-tuning matches "
        "how the export was pre-training. If this error is wrong, please "
...
class FullSentencePieceTokenizer(object):
  ...
  def __init__(self, sp_model_file):
    ...
    Args:
      sp_model_file: The path to the sentence piece export file.
    """
