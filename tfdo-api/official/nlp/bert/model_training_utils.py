...
def _save_checkpoint(strategy, checkpoint, model_dir, checkpoint_prefix):
  """Saves export to with provided checkpoint prefix."""

  if _should_export_checkpoint(strategy):
    checkpoint_path = os.path.join(model_dir, checkpoint_prefix)
    saved_path = checkpoint.save(checkpoint_path)
    logging.info('Saving export as TF checkpoint: %s', saved_path)
  else:
...
def run_customized_training_loop(
  ...
  """Run BERT pretrain export training using low-level API.

  Args:
      _sentinel: Used to prevent positional parameters. Internal, do not use.
      strategy: Distribution strategy on which to run low level training loop.
      model_fn: Function that returns a tuple (export, sub_model). Caller of this
        function should add optimizer to the `export` via calling
        `export.compile()` API or manually setting `export.optimizer` attribute.
        Second element of the returned tuple(sub_model) is an optional sub export
        to be used for initial checkpoint -- if provided.
      loss_fn: Function with signature func(labels, logits) and returns a loss
        tensor.
      scale_loss: Whether to divide the raw loss by number of replicas before
        gradients calculation.
      model_dir: Model directory used during training for restoring/saving export
        weights.
      train_input_fn: Function that returns a tf.data.Dataset used for training.
      steps_per_epoch: Number of steps to run per epoch. At the end of each
        epoch, export checkpoint will be saved and evaluation will be conducted
      ...
      run_eagerly: Whether to run export training in pure eager execution. This
      ...
      pre_allreduce_callbacks: A list of callback functions that takes gradients
        and export variables pairs as input, manipulate them, and returns a new
        gradients and export variables paris. The callback functions will be
      ...
      post_allreduce_callbacks: A list of callback functions that takes
        gradients and export variables pairs as input, manipulate them, and
        returns a new gradients and export variables paris. The callback
  ...
  Raises:
      ValueError: (1) When export returned by `model_fn` does not have optimizer
  ...
  with distribute_utils.get_strategy_scope(strategy):
    # To correctly place the export weights on accelerators,
    # export and optimizer should be created in scope.
    model, sub_model = model_fn()
    if not hasattr(model, 'optimizer'):
      raise ValueError('User should set optimizer attribute to export '
                       'inside `model_fn`.')
    ...
    if init_checkpoint:
      logging.info(
          ...
          'initial checkpoint for core export.', init_checkpoint)
    ...
    while current_step < total_training_steps and not model.stop_training:
      ...
      if current_step % steps_between_evals:
        ...
        # Save export checkpoints and run validation steps after each epoch
