...
def train(
  ...
  Args:
      ...
      steps_per_loop: Number of steps per graph-mode loop. In order to reduce
        ...
        steps_per_loop.
      optimizer: The optimizer for export.
      ...
      init_from_transformerxl: Whether to load to `transformerxl_model` of
        `model_fn`.
      model_dir: The directory of export (checkpoints, summaries).
      save_steps: The frequency to save checkpoints. Every save_steps, we save a
        export checkpoint. Model checkpoint will be saved and evaluation will be
  ...
  Raises:
    TypeError: if export directory is not specified.
  """
  required_arguments = [
    ...
    def _replicated_step(inputs, mem=None):
      ...
      if input_meta_data["lr_layer_decay_rate"] != 1.0:
        ...
        for i in range(len(clipped)):
          m = re.search(r"export/transformer/layer_(\d+?)/", tvars[i].name)
          if not m:
            continue
        ...
        for i in range(len(clipped)):
          for l in range(n_layer):
            if "export/transformer/layer_{}/".format(l) in tvars[i].name:
