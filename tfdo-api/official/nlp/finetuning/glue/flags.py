...
def define_flags():
  ...
  flags.DEFINE_enum(
      ...
      'run prediction using the export in `model_dir`.')
  ...
  flags.DEFINE_string('model_dir', '', 'The export directory containing '
  ...
  flags.DEFINE_string('init_checkpoint', '',
                      'Initial checkpoint from a pre-trained BERT export.')

  flags.DEFINE_string(
      'model_config_file', '', 'The config file specifying the architecture '
      'of the pre-trained export. The file can be either a bert_config.json '
      'file or `encoders.EncoderConfig` in yaml file.')

  flags.DEFINE_string(
      'hub_module_url', '', 'TF-Hub path/url to a pretrained export. If '
