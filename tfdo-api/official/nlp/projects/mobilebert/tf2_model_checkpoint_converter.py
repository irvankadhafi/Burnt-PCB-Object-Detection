...
flags.DEFINE_boolean("use_model_prefix", False,
                     ("If use export name as prefix for variables. Turn this"
                      "flag on when the converted checkpoint is used for export"
                      "in subclass implementation, which uses the export name as"
...
def convert(checkpoint_from_path,
  ...
  Args:
    ...
    bert_config: A `BertConfig` to create the core export.
...
def create_v2_checkpoint(model, src_checkpoint, output_path):
  ...
  # Uses streaming-restore in eager export to read V1 name-based checkpoints.
