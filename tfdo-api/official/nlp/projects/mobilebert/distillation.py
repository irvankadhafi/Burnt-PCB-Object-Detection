...
"""Progressive distillation for MobileBERT student export."""
...
class LayerWiseDistillConfig(base_config.Config):
  ...
  # The ids of teacher layers that will be mapped to the student export.
  ...
  # and each layer of teacher export will be mapped to student's corresponding
  # layer.
...
class BertDistillationTaskConfig(cfg.TaskConfig):
  """Defines the teacher/student export architecture and training data."""
  ...
  student_model: bert.PretrainerConfig = bert.PretrainerConfig(
      encoder=encoders.EncoderConfig(type='mobilebert'))
  # The path to the teacher export checkpoint or its directory.
...
class BertDistillationTask(policies.ProgressivePolicy, base_task.Task):
  ...
  def __init__(self,
    ...
    # Build the teacher and student pretrainer export.
  ...
  def build_model(self, stage_id) -> tf.keras.Model:
    ...
    if stage_id != self.num_stages() - 1:
      # Build a export that outputs teacher's and student's transformer outputs.
      ...
      # Build a export that outputs teacher's and student's MLM/NSP outputs.
  ...
  def train_step(self, inputs, model: tf.keras.Model,
    ...
    Args:
      ...
      model: the export, forward pass definition.
  ...
  def cur_checkpoint_items(self):
    """Checkpoints for export, stage_id, optimizer for preemption handling."""
    return dict(
  ...
  def initialize(self, model):
    ...
    # This function will be called when no checkpoint found for the export,
    # i.e., when the training starts (not preemption case).
    # The weights of teacher pretrainer and student pretrainer will be
    # initialized, rather than the passed-in `export`.
    del model
    logging.info('Begin to load checkpoint for teacher pretrainer export.')
    ...
    logging.info('Begin to copy word embedding from teacher export to student.')
