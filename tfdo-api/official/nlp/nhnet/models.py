...
class Bert2Bert(tf.keras.Model):
  """Bert2Bert encoder decoder export for training."""
...
class NHNet(Bert2Bert):
  """NHNet export which performs multi-doc decoding."""
...
def get_bert2bert_layers(params: configs.BERT2BERTConfig):
  """Creates a Bert2Bert stem export and returns Bert encoder/decoder.

  We use funtional-style to create stem export because we need to make all layers
...
def create_transformer_model(params,
  ...
  """A helper to create Transformer export."""
...
def create_bert2bert_model(
  ...
  """A helper to create Bert2Bert export."""
  bert_layer, decoder_layer = get_bert2bert_layers(params=params)
  if init_checkpoint:
...
def create_nhnet_model(
    ...
    cls=NHNet,
    init_checkpoint: Optional[Text] = None) -> tf.keras.Model:
  """A helper to create NHNet export."""
...
def get_model_params(model: Optional[Text] = "bert2bert",
  ...
  if model == "bert2bert":
    ...
    raise KeyError("The export type is not defined: %s" % model)
...
def create_model(model_type: Text,
  ...
  if model_type == "bert2bert":
    ...
    raise KeyError("The export type is not defined: %s" % model_type)
