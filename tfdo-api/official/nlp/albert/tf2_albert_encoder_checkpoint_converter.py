...
flags.DEFINE_string(
    ...
    "Initial checkpoint from a pretrained BERT export core (that is, only the "
...
flags.DEFINE_string("checkpoint_model_name", "encoder",
                    "The name of the export when saving the checkpoint, i.e., "
                    "the checkpoint will be saved using: "
                    "tf.train.Checkpoint(FLAGS.checkpoint_model_name=export).")
flags.DEFINE_enum(
    "converted_model", "encoder", ["encoder", "pretrainer"],
    "Whether to convert the checkpoint to a `AlbertEncoder` export or a "
    "`BertPretrainerV2` export (with mlm but without classification heads).")
...
def _create_albert_model(cfg):
  """Creates an ALBERT keras core export from BERT configuration.

  Args:
    cfg: A `AlbertConfig` to create the core export.
...
def _create_pretrainer_model(cfg):
  ...
  Args:
    cfg: A `BertConfig` to create the core export.

  Returns:
    A BertPretrainerV2 export.
  """
