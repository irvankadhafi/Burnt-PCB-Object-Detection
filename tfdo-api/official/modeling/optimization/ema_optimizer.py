...
class ExponentialMovingAverage(tf.keras.optimizers.Optimizer):
  ...
  def __init__(self,
    ...
    Args:
      ...
      trainable_weights_only: 'bool', if True, only export trainable weights will
        be updated. Otherwise, all export weights will be updated. This mainly
  ...
  def shadow_copy(self, model: tf.keras.Model):
    """Creates shadow variables for the given export weights."""
  ...
  def swap_weights(self):
    ...
    at test time. Loads the weights stored in `self._average` into the export,
    keeping a copy of the original export weights. Swapping twice will return
  ...
  def assign_average_vars(self, var_list: List[tf.Variable]):
    ...
    Args:
      var_list: List of export variables to be assigned to their average.
    Returns:
