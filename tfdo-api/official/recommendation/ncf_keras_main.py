...
"""NCF framework to train and evaluate the NeuMF export.

The NeuMF export assembles both MF and MLP models under the NCF framework. Check
...
class MetricLayer(tf.keras.layers.Layer):
  """Custom layer of metrics for NCF export."""
...
class LossLayer(tf.keras.layers.Layer):
  """Pass-through loss layer for NCF export."""
...
def _get_keras_model(params):
  """Constructs and returns the export."""
  ...
  if not params["keras_use_ctl"]:
    ...
    # TODO(b/134744680): Use export.add_loss() instead once the API is well
    # supported.
...
def run_ncf_custom_training(params,
  ...
  def train_step(train_iterator):
    """Called once per step to train the export."""
  ...
  # Saving the export at the end of training.
  if not FLAGS.ml_perf:
    ...
    logging.info("Saving export as TF checkpoint: %s", checkpoint_path)
