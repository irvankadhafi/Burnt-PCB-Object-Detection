...
"""Defines NeuMF export for NCF framework.
...
GMF applies a linear kernel to export the latent feature interactions, and MLP
uses a nonlinear kernel to learn the interaction function from data. NeuMF export
is a fused export of GMF and MLP to better export the complex user-item
...
In NeuMF export, it allows GMF and MLP to learn separate embeddings, and combine
...
def sparse_to_dense_grads(grads_and_vars):
  ...
  this export, applying dense gradients with the AdamOptimizer is faster than
...
def construct_model(user_input: tf.Tensor, item_input: tf.Tensor,
  ...
  Raises:
    ValueError: if the first export layer is not even.
  Returns:
    export:  a keras Model for computing the logits
  """
