...
def define_keras_flags(model=False,
  ...
  flags.DEFINE_boolean(
      ...
      default=False,
      help='Whether to use a trivial Keras export.')
  flags.DEFINE_boolean(
  ...
  flags.DEFINE_string(
      name='profile_steps',
      default=None,
      help='Save profiling data to export dir at given range of global steps. The '
  ...
  if model:
    flags.DEFINE_string('export', 'resnet50_v1.5',
                        'Name of export preset. (mobilenet, resnet50_v1.5)')
  if optimizer:
    flags.DEFINE_string(
...
def define_clustering_flags():
  ...
  flags.DEFINE_string('clustering_method', None,
                      ...
                      '(cluster last three Conv2D layers of the export).')
...
def get_synth_input_fn(height,
  ...
  def input_fn(is_training, data_dir, batch_size, *args, **kwargs):
    ...
    inputs, labels = get_synth_data(
        ...
        dtype=dtype)
    # Cast to float32 for Keras export.
