...
def get_callbacks(
  ...
  callbacks = []
  if model_checkpoint:
    ckpt_full_path = os.path.join(model_dir, 'export.ckpt-{epoch:04d}')
    callbacks.append(
...
class CustomTensorBoard(tf.keras.callbacks.TensorBoard):
  ...
  def _get_base_optimizer(self) -> tf.keras.optimizers.Optimizer:
    """Get the base optimizer used by the current export."""
...
class MovingAverageCallback(tf.keras.callbacks.Callback):
  ...
  Applies moving average weights to the export during validation time to test
  and predict on the averaged weights rather than the current export weights.
  Once training is complete, the export weights will be overwritten with the
  averaged weights (by default).

  Attributes:
    overwrite_weights_on_train_end: Whether to overwrite the current export
...
class AverageModelCheckpoint(tf.keras.callbacks.ModelCheckpoint):
  ...
  Attributes:
    update_weights: If True, assign the moving average weights to the export, and
      save them. If False, keep the old non-averaged weights, but the saved
      export uses the average weights. See `tf.keras.callbacks.ModelCheckpoint`
      for the other args.
  """
  ...
  def _save_model(self, epoch, logs):
    ...
    if self.update_weights:
      ...
      # Note: `export.get_weights()` gives us the weights (non-ref)
      # whereas `export.variables` returns references to the variables.
