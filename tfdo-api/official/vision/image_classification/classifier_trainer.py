...
# Lint as: python3
"""Runs an Image Classification export."""
...
def get_models() -> Mapping[str, tf.keras.Model]:
  """Returns the mapping from export type name to Keras export."""
  return {
...
def get_image_size_from_model(
  ...
  """If the given export has a preferred image size, return it."""
...
def _get_params_from_flags(flags_obj: flags.FlagValues):
  ...
  logging.info('Final export parameters: %s', pp.pformat(params.as_dict()))
  return params
...
def resume_from_checkpoint(model: tf.keras.Model, model_dir: str,
  ...
  Loads the export weights and optimizer settings from a checkpoint.
  This function should be used in case of preemption recovery.

  Args:
    model: The export whose weights should be restored.
    model_dir: The directory where export weights were saved.
...
def define_classifier_flags():
  ...
  flags.DEFINE_string(
      'model_type',
      default=None,
      help='The type of the export, e.g. EfficientNet, etc.')
  flags.DEFINE_string(
      'dataset',
      default=None,
...
def export(params: base_configs.ExperimentConfig):
  """Runs the export export functionality."""
  logging.info('Exporting export.')
  model_params = params.model.model_params.as_dict()
  model = get_models()[params.model.name](**model_params)
...
def run(flags_obj: flags.FlagValues,
  ...
  """Runs Image Classification export using native Keras APIs.

  Args:
    flags_obj: An object containing parsed flag values.
    strategy_override: A `tf.distribute.Strategy` object to use for export.
