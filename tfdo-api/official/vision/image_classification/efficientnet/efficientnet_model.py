...
# Lint as: python3
"""Contains definitions for EfficientNet export.
...
def mb_conv_block(inputs: tf.Tensor,
  ...
  Args:
    ...
    config: ModelConfig, a set of export parameters
...
class EfficientNet(tf.keras.Model):
  """Wrapper class for an EfficientNet Keras export.

  Contains helper methods to build, manage, and save metadata about the export.
  """

  def __init__(self,
               config: Optional[ModelConfig] = None,
               overrides: Optional[Dict[Text, Any]] = None):
    """Create an EfficientNet export.

    Args:
      config: (optional) the main export parameters to create the export
    ...
    # Cast to float32 in case we have a different export dtype
    output = tf.cast(output, tf.float32)

    logging.info('Building export %s with params %s', model_name, self.config)
  ...
  def from_name(cls,
                ...
                overrides: Optional[Dict[Text, Any]] = None):
    """Construct an EfficientNet export from a predefined export name.

    E.g., `EfficientNet.from_name('efficientnet-b0')`.

    Args:
      model_name: the predefined export name
      model_weights_path: the path to the weights (h5 file or saved export dir)
      weights_format: the export weights format. One of 'saved_model', 'h5', or
        'checkpoint'.
      overrides: (optional) a dict containing keys that can override config
    ...
    if model_name not in model_configs:
      raise ValueError('Unknown export name {}'.format(model_name))
