...
flags.DEFINE_string(
    'model_id', 'a0', 'MoViNet export name.')
flags.DEFINE_bool(
    'causal', False, 'Run the export in causal mode.')
flags.DEFINE_string(
    'conv_type', '3d',
...
def main(_) -> None:
  ...
  # states. These dimensions will be set to `None` once the export is built.
  ...
  # Compile export to generate some internal Keras variables.
  model.compile()
  ...
  if FLAGS.causal:
    # Call the export once to get the output states. Call again with `states`
