...
def block_group(inputs: tf.Tensor,
                ...
                num_frames=32,
                temporal_dilation=1):
  """Creates one group of blocks for the AssembleNett export.

  Args:
    inputs: `Tensor` of size `[batch*time, channels, height, width]`.
    filters: `int` number of filters for the first convolution of the layer.
    block_fn: `function` for the block to use within the export
...
class _ApplyEdgeWeight(layers.Layer):
  ...
  def __init__(self,
    ...
    Args:
      ...
      model_edge_weights: AssembleNet export structure connection weights in the
        string format.
...
class AssembleNet(tf.keras.Model):
  ...
  def __init__(
    ...
    Args:
      block_fn: `function` for the block to use within the export. Currently only
      ...
      model_structure: AssembleNet export structure in the string format.
...
def assemblenet_v1(assemblenet_depth: int,
                   ...
                   combine_method: str = 'sigmoid',
                   **kwargs):
  """Returns the AssembleNet export for a given size and number of output classes."""
