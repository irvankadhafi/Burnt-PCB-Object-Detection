...
# Lint as: python3
"""Detection input and export functions for serving/inference."""
...
class DetectionModule(export_base.ExportModule):
  ...
  def _build_model(self):
    ...
    if isinstance(self.params.task.model, configs.maskrcnn.MaskRCNN):
      ...
      raise ValueError('Detection module not implemented for {} export.'.format(
          type(self.params.task.model)))

    return model

  def _build_inputs(self, image):
    """Builds detection export inputs for serving."""
  ...
  def serve(self, images: tf.Tensor):
    ...
    # To overcome keras.Model extra limitation to save a export with layers that
    # have multiple inputs, we use `export.call` here to trigger the forward
    ...
    if self.params.task.model.detection_generator.apply_nms:
      # For RetinaNet export, apply export_config.
      ...
      if isinstance(self.params.task.model, configs.retinanet.RetinaNet):
        ...
        # export inference to work on chain (go/chain) as chain requires floating
        # point outputs.
