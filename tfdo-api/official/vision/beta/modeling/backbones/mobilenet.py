...
def block_spec_decoder(
  ...
  Args:
    ...
    filter_size_scale: A `float` multiplier for the filter size for all
      ...
      computation cost of the export.
    divisible_by: An `int` that ensures all inner dimensions are divisible by
      this number.
    finegrain_classification_mode: If True, the export will keep the last layer
...
class MobileNet(tf.keras.Model):
  """Creates a MobileNet family export."""
  ...
  def __init__(
      ...
      finegrain_classification_mode: bool = True,
      **kwargs):
    """Initializes a MobileNet export.
