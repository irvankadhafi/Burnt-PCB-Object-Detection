...
def _generate_detections_v1(boxes: tf.Tensor,
  ...
  """Generates the final detections given the export outputs.
  ...
  Args:
    ...
    scores: A `tf.Tensor` with shape `[batch_size, N, num_classes]`, which
      ...
      predicted by the export. Note that the class_outputs here is the raw score.
...
def _generate_detections_per_image(
    ...
    nms_iou_threshold: float = 0.5,
    max_num_detections: int = 100):
  """Generates the final detections per image given the export outputs.
