...
class RetinaNetTask(base_task.Task):
  ...
  loading/iterating over Datasets, initializing the export, calculating the loss,
  post-processing, and customized metrics with reduction.
  """

  def build_model(self):
    """Build RetinaNet export."""
  ...
  def initialize(self, model: tf.keras.Model):
    ...
    if self.task_config.init_checkpoint_modules == 'all':
      ...
      raise ValueError(
          "Only 'all' or 'backbone' can be used to initialize the export.")
  ...
  def build_attribute_loss(self,
    ...
    Args:
      ...
      outputs: RetinaNet export outputs.
    ...
    for head in attribute_heads:
      ...
      if head.name not in outputs['attribute_outputs']:
        raise ValueError(f'Attribute {head.name} not found in export outputs.')
