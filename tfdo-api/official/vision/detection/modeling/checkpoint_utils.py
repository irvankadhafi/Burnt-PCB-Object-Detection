...
checkpoint to Tensorflow 2.x (keras) export.
"""
...
def _build_assignment_map(keras_model,
  ...
  export. Variable names are remapped from the original TPUEstimator export to
  the new Keras name.
...
def make_restore_checkpoint_fn(checkpoint_path, prefix='', skip_regex=None):
  ...
  Args:
    checkpoint_path: path of the checkpoint folder or file.
      ...
      Example 2: '/path/to/export.ckpt-22500'
    prefix: prefix in the variable name to be remove for alignment with names in
      the checkpoint.
  ...
  Returns:
    Callable[tf.kears.Model] -> void. Fn to load v1 checkpoint to keras export.
  """

  def _restore_checkpoint_fn(keras_model):
    """Loads pretrained export through scaffold function."""
