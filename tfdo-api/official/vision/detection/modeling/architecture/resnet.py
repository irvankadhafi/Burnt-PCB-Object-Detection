...
class Resnet(object):
  """Class to build ResNet family export."""
  ...
  def __init__(
    ...
    Args:
      resnet_depth: `int` depth of ResNet backbone export.
  ...
  def __call__(self, inputs, is_training=None):
    """Returns the ResNet export for a given size and number of output classes.

    Args:
      inputs: a `Tesnor` with shape [batch_size, height, width, 3] representing
        a batch of images.
      is_training: `bool` if True, the export is in training mode.
  ...
  def block_group(self, inputs, filters, block_fn, blocks, strides, name,
                  is_training):
    """Creates one group of blocks for the ResNet export.
  ...
  def resnet_v1_generator(self, block_fn, layers):
    ...
    Args:
      block_fn: `function` for the block to use within the export. Either
    ...
    Returns:
      ...
      output `Tensor` of the ResNet export.
    """
