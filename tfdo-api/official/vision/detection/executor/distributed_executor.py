...
def _save_checkpoint(checkpoint, model_dir, checkpoint_prefix):
  """Saves export to model_dir with provided checkpoint prefix."""
...
class SummaryWriter(object):
  ...
  def __init__(self, model_dir: Text, name: Text):
    ...
    Args:
      model_dir: the export folder path.
...
class DistributedExecutor(object):
  ...
  def __init__(self, strategy, params, model_fn, loss_fn, is_multi_host=False):
    ...
    Args:
      ...
      model_fn: Keras export function. Signature:
  ...
  def _create_replicated_step(self,
    ...
    Args:
      strategy: an instance of tf.distribute.Strategy.
      model: (Tensor, bool) -> Tensor. export function.
  ...
  def train(
    ...
    Args:
      ...
      eval_input_fn: (Optional) same type as train_input_fn. If not None, will
        ...
        step.
      model_dir: the folder path for export checkpoints.
    ...
    while current_step < total_steps:
      ...
      # Saves export checkpoints and run validation steps at every
      # iterations_per_loop steps.
      # To avoid repeated export saving, we do not save after the last
      # step of training.
  ...
  def evaluate_from_model_dir(
      ...
      summary_writer_fn: Callable[[Text, Text], SummaryWriter] = SummaryWriter):
    """Runs distributed evaluation on export folder.

    Args:
      model_dir: the folder for storing export checkpoints.
      eval_input_fn: (Optional) same type as train_input_fn. If not None, will
    ...
    # Read checkpoints from the given export directory
...
class ExecutorBuilder(object):
  ...
  def build_executor(self,
    ...
    Args:
      ...
      params: ParamsDict, all the export parameters and runtime parameters.
      model_fn: Keras export function.
