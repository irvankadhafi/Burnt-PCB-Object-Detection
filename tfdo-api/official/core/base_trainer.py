...
interchangable and independent on export architectures and tasks.
"""
import functools
...
class Recovery:
  """Built-in export blowup recovery module.

  Checks the loss value by the given threshold. If applicable, recover the
  export by reading the checkpoint on disk.
  """
  ...
  def maybe_recover(self, loss_value, global_step):
    ...
    logging.warning(
        "Recovering the export from checkpoint: %s. The loss value becomes "
...
class Trainer(_AsyncTrainer):
  ...
  def __init__(
    ...
    Args:
      ...
      model: The export instance, e.g. a tf.keras.Model instance.
  ...
  def train_loop_end(self):
    """See base class."""
    self.join()
    # Checks if the export numeric status is stable and conducts the checkpoint
