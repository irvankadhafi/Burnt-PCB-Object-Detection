...
class ActionsTest(tf.test.TestCase, parameterized.TestCase):
  ...
  def test_ema_checkpointing(self, distribution):
    with distribution.scope():
      ...
      # Creats average weights for the export variables. Average weights are
      # initialized to zero.
      optimizer.shadow_copy(model)
      checkpoint = tf.train.Checkpoint(model=model)

      # Changes export.value to 3, average value is still 0.
      model.value.assign(3)

      # Checks export.value is 3
      ...
      # Checks export.value is 0 after swapping.
