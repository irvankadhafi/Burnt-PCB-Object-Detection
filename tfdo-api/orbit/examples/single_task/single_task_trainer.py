...
class SingleTaskTrainer(orbit.StandardTrainer):
  """Trains a single-output export on a given dataset.

  This trainer will handle running a export with one output on a single
  dataset. It will apply the provided loss function to the export's output
  to calculate gradients and will apply them via the provided optimizer. It will
  also supply the output of that export to one or more `tf.keras.metrics.Metric`
  objects.
  """
  ...
  def __init__(self,
    ...
    If the `SingleTaskTrainer` should run its export under a distribution
    ...
    Arguments:
      ...
      label_key: The key corresponding to the label value in feature
        ...
        from the dictionary before it is passed to the export.
  ...
  def train_step(self, iterator):
    ...
    def train_fn(inputs):
      with tf.GradientTape() as tape:
        ...
        # the export never sees it.
        target = inputs.pop(self.label_key)

        # Get the outputs of the export.
        ...
        # Get the gradients by applying the loss to the export's trainable
        # variables.
