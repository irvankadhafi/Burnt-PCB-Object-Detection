...
class SingleTaskEvaluator(orbit.StandardEvaluator):
  """Evaluates a single-output export on a given dataset.

  This evaluator will handle running a export with one output on a single
  dataset, and will apply the output of that export to one or more
  ...
  def __init__(self,
    ...
    If the `SingleTaskEvaluator` should run its export under a distribution
    ...
    Arguments:
      ...
      label_key: The key corresponding to the label value in feature
        ...
        the dictionary before it is passed to the export.
