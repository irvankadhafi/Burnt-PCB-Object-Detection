...
class Controller:
  """Class that controls the outer loop of export training and evaluation.
  ...
  There are four top-level "outer loops" provided:
    ...
    - `evaluate`, for one-off export evaluation;
    ...
    - `evaluate_continuously`, for monitoring a given directory and running
      evaluations on new export checkpoints.
  ...
  metrics to Vizier, export exporting, additional logging, etc. See the
  ...
  def __init__(
    ...
    the associated export directory, the export will be restored from the most
    ...
    Args:
      global_step: An integer `tf.Variable` storing the global training step
        ...
        the export's optimizer (e.g. `trainer.optimizer.iterations`). In cases
        where multiple optimizers are used, or if one export "step" corresponds
        to more than one update to export parameters, users can create and
      ...
      checkpoint_manager: An instance of `tf.train.CheckpointManager`. If
        provided and there are checkpoints in the associated export directory,
        the export will be restored from the most recent checkpoint inside this
  ...
  def train_and_evaluate(self,
    ...
    training the export until the global step count equals `train_steps`, and
  ...
  def restore_checkpoint(self, checkpoint_path: Optional[str] = None):
    """Restores the export from a checkpoint.
    ...
    Args:
      checkpoint_path: An optional string specifying the checkpoint path to
        ...
        (or initialize the export using a custom `init_fn` if no checkpoints can
    ...
    with self.strategy.scope():
      ...
      if checkpoint_path is not None:
        _log(f"restoring export from {checkpoint_path}...")
        self.checkpoint_manager.checkpoint.restore(checkpoint_path)
      else:
        _log("restoring or initializing export...")
        checkpoint_path = self.checkpoint_manager.restore_or_initialize()

    if checkpoint_path is not None:
      _log(f"restored export from {checkpoint_path}.")
    else:
      _log("initialized export.")

    return checkpoint_path

  def save_checkpoint(self):
    """Saves the export to a checkpoint.
