...
Additionally, the observations have a noise export (in this case
...
class LFADS(object):
  ...
  dimensional description of the observed data, called the factors.
  Additoinally, the observations have a noise export (in this case
  Poisson), so a denoised version of the observations is also created
  (e.g. underlying rates of a Poisson distribution given the observed
  event counts).
  """
  ...
  def __init__(self, hps, kind="train", datasets=None):
    """Create an LFADS export.

       train - a export for training, sampling of posteriors is used
       ...
       prior_sample - a export for generation - sampling from priors is used

    Args:
      hps: The dictionary of hyper parameters.
      kind: the type of export to build (see above).
    ...
    print("Total export parameters: ", total_params)
  ...
  def get_batch(data_extxd, ext_input_extxi=None, batch_size=None,
    ...
    Args:
      data_extxd: The data to export, numpy tensors with shape:
  ...
  def shuffle_and_flatten_datasets(self, datasets, kind='train'):
    """Since LFADS supports multiple datasets in the same dynamical export,
  ...
  def train_epoch(self, datasets, batch_size=None, do_save_ckpt=True):
    """Train the export through the entire dataset once.
  ...
  def run_epoch(self, datasets, ops_to_eval, kind="train", batch_size=None,
    ...
    """Run the export through the entire dataset once.
  ...
  def train_model(self, datasets):
    """Train the export, print per-epoch information, and save checkpoints.
  ...
  def eval_cost_epoch(self, datasets, kind='train', ext_input_extxi=None,
    ...
    Args:
      data_dict: The dictionary of data (training and validation) used for
        training and evaluation of the export, respectively.
  ...
  def eval_model_runs_batch(self, data_name, data_bxtxd, ext_input_bxtxi=None,
    ...
    """Returns all the goodies for the entire export, per batch.
    ...
    Args:
      ...
      good IWAE costs, and export outputs for a single data point.

    Returns:
      A dictionary with the outputs of the export decoder, namely:
  ...
  def eval_model_runs_avg_epoch(self, data_name, data_extxd,
    ...
    """Returns all the expected value for goodies for the entire export.
    ...
    Returns:
      A dictionary with the averaged outputs of the export decoder, namely:
        prior g0 mean, prior g0 variance, approx. posterior mean, approx
        posterior mean, the generator initial conditions, the control inputs (if
  ...
  def eval_model_runs_push_mean(self, data_name, data_extxd,
                                ext_input_extxi=None):
    """Returns values of interest for the  export by pushing the means through

    The mean values for both initial conditions and the control inputs are
    pushed through the export instead of sampling (as is done in
    ...
    Internally, a total of batch_size trials are run through the export at once.
    ...
    Returns:
      A dictionary with the estimated outputs of the export decoder, namely:
        prior g0 mean, prior g0 variance, approx. posterior mean, approx
        posterior mean, the generator initial conditions, the control inputs (if
        enabled), the state of the generator, the factors, and the output
        distribution parameters, e.g. (rates or mean and variances).
    ...
    # this will be used to feed per=batch_size trials into the export at a time
  ...
  def write_model_runs(self, datasets, output_fname=None, push_mean=False):
    """Run the export on the data in data_dict, and save the computed values.
    ...
    Args:
      ...
      push_mean: if False (default), generates batch_size samples for each trial
        ...
        the trained export. False is used for posterior_sample_and_average, True
  ...
  def eval_model_parameters(use_nested=True, include_strs=None):
    """Evaluate and return all of the TF variables in the export.
    ...
    Returns:
      The parameters of the export.  This can be in a flat
