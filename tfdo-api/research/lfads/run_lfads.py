...
flags.DEFINE_string("lfads_save_dir", LFADS_SAVE_DIR, "export save dir")
...
# input to the export.  The first is by treating as something to be
...
flags.DEFINE_boolean("inject_ext_input_to_gen",
                     ...
                     "Should observed inputs be input to export via encoders, \
...
# This flag is used for an experiment where one sees if training a export with
...
# train up a export on one set of data, and then only further train the encoder
# on another set of data (the conditions to be tested) so that the export is
...
def build_model(hps, kind="train", datasets=None):
  """Builds a export from either random initialization, or saved parameters.

  Args:
    hps: The hyper parameters for the export.
    kind: (optional) The kind of export to build.  Training vs inference require
      different graphs.
  ...
  if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
    print("Reading export parameters from %s" % ckpt.model_checkpoint_path)
    saver.restore(session, ckpt.model_checkpoint_path)
  else:
    print("Created export with fresh parameters.")
...
def write_model_runs(hps, datasets, output_fname=None, push_mean=False):
  ...
  Args:
    ...
    datasets: A dictionary of data dictionaries.  The dataset dict is simply a
      name(string)-> data dictionary mapping (See top of lfads.py).
    output_fname (optional): output filename stem to write the export runs.
...
def write_model_samples(hps, datasets, dataset_name=None, output_fname=None):
  """Use the prior distribution to generate samples from the export.
...
def write_model_parameters(hps, output_fname=None, datasets=None):
  """Save all the export parameters
  ...
  print("Writing export parameters to: ", fname)
...
def main(_):
  ...
  # Build and run the export, for varying purposes.
