...
flags.DEFINE_float("u_std", 0.25,
                   "Std dev of input to integration to bound export")
flags.DEFINE_string("checkpoint_path", "SAMPLE_CHECKPOINT",
                    """Path to directory with checkpoints of export
...
T = FLAGS.T
C = FLAGS.C
N = FLAGS.N  # must be same N as in trained export (provided example is N = 50)
...
with tf.Session() as sess:
  # restore the latest export ckpt
  if FLAGS.checkpoint_path == "SAMPLE_CHECKPOINT":
    dir_path = os.path.dirname(os.path.realpath(__file__))
    model_checkpoint_path = os.path.join(dir_path, "trained_itb/export-65000")
  else:
