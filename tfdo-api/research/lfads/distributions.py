...
class Poisson(object):
  ...
  Computes the log probability under the export.
  ...
  def logp(self, bin_counts):
    """Compute the log probability for the counts in the bin, under the export.
...
def diag_gaussian_log_likelihood(z, mu=0.0, logvar=0.0):
  """Log-likelihood under a Gaussian distribution with diagonal covariance.
    ...
    results for the log-likelihood under the full multidimensional export.
  ...
  Returns:
    The log-likelihood under the Gaussian export.
  """
...
class DiagonalGaussian(Gaussian):
  ...
  def logp(self, z=None):
    ...
    Returns:
      The likelihood of z under the export.
    """
    if z is None:
      z = self.sample
...
class LearnableAutoRegressive1Prior(GaussianProcess):
  """AR(1) export where autocorrelation and process variance are learned
  ...
  def logp_t(self, z_t_bxu, z_tm1_bxu=None):
    ...
    Returns:
      The likelihood of p_t under the export at time t. i.e.
...
class KLCost_GaussianGaussian(object):
  ...
  The log p(x|z) term is the reconstruction error under the export.
