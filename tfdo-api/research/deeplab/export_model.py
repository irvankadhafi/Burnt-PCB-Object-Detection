...
# ==============================================================================
"""Exports trained export to TensorFlow frozen graph."""
...
# Input name of the exported export.
...
def _create_input_tensors():
  """Creates and prepares input tensors for DeepLab export.
...
def main(unused_argv):
  ...
  tf.logging.info('Prepare to export export to: %s', FLAGS.export_path)
  ...
  with tf.Graph().as_default():
    ...
    if tuple(FLAGS.inference_scales) == (1.0,):
      tf.logging.info('Exported export performs single-scale inference.')
      predictions = model.predict_labels(
          image,
          model_options=model_options,
          image_pyramid=FLAGS.image_pyramid)
    else:
      tf.logging.info('Exported export performs multi-scale inference.')
