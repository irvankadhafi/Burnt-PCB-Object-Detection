...
def get_inputs(input_queue, num_classes, merge_multiple_label_boxes=False):
  """Dequeues batch and constructs inputs to object detection export.
...
def get_restore_checkpoint_ops(restore_checkpoints, detection_model,
  ...
  Args:
    ...
    detection_model: Object detection export built from config file.
    train_config: a train_pb2.TrainConfig protobuf.

  Returns:
    restorers: A list ops to init the export from checkpoints.
...
def train(create_tensor_dict_fn,
  ...
  with tf.Graph().as_default():
    ...
    # Create ops required to initialize the export from a given checkpoint.
    init_fn = None
