...
class ModelBuilderTest(tf.test.TestCase):
  ...
  def get_model_configs_from_proto(self):
    """Creates a export text proto for testing.

    Returns:
      A dictionary of export configs.
    """
  ...
  def get_interleaved_model_configs_from_proto(self):
    """Creates an interleaved export text proto for testing.
  ...
  def test_model_creation_from_valid_configs(self):
    ...
    self.assertEqual(configs['export'].ssd.negative_class_weight, 2.0)
    self.assertTrue(configs['export'].ssd.normalize_loc_loss_by_codesize)
    self.assertEqual(configs['export'].ssd.feature_extractor.type,
                     'lstm_ssd_mobilenet_v1')

    model = self.create_train_model(configs['export'], configs['lstm_model'])
    ...
    model = self.create_eval_model(configs['export'], configs['lstm_model'])
    # Test architechture type.
    self.assertIsInstance(model, lstm_ssd_meta_arch.LSTMSSDMetaArch)
    # Test LSTM configs.
    self.assertEqual(model.unroll_length, 4)
  ...
  def test_model_creation_from_invalid_configs(self):
    configs = self.get_model_configs_from_proto()
    # Test export build failure with wrong input configs.
    with self.assertRaises(AttributeError):
      _ = self.create_train_model(configs['export'], configs['export'])
    with self.assertRaises(AttributeError):
      _ = self.create_eval_model(configs['export'], configs['export'])
