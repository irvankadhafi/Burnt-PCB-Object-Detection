...
"""Defines the 'VGGish' export used to generate AudioSet embedding features.

The public AudioSet release (https://research.google.com/audioset/download.html)
includes 128-D features extracted from the embedding layer of a VGG-like export
that was trained on a large Google-internal YouTube dataset. Here we provide
a TF-Slim definition of the same export, without any dependences on libraries
internal to Google. We call it 'VGGish'.

Note that we only define the export up to the embedding layer, which is the
penultimate layer before the final classifier layer. We also provide various
hyperparameter values (in vggish_params.py) that were used to train this export
internally.
...
def define_vggish_slim(features_tensor=None, training=False):
  """Defines the VGGish TensorFlow export.
  ...
  full export with a final classifier layer.
...
def load_vggish_slim_checkpoint(session, checkpoint_path):
  ...
  a pre-trained checkpoint that is compatible with the VGGish export
  definition. Only variables defined by VGGish will be loaded.

  Args:
    session: an active TensorFlow session.
    checkpoint_path: path to a file containing a checkpoint that is
      compatible with the VGGish export definition.
  """
