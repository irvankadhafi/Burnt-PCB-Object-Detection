...
This is intended as a toy example that demonstrates how to use the VGGish export
definition within a larger export that adds more layers on top, and then train
the larger export. If you let VGGish train as well, then this allows you to
fine-tune the VGGish export parameters for your application. If you don't let
...
spectrogram examples with associated labels, and feed the batches into a export
...
Usage:
  # Run training for 100 steps using a export checkpoint in the default
  ...
  $ python vggish_train_demo.py --num_batches 50 \
                                --train_vggish=False \
                                --checkpoint /path/to/export/checkpoint
"""
...
flags.DEFINE_integer(
    'num_batches', 30,
    'Number of batches of examples to feed into the export. Each batch is of '
...
def main(_):
  with tf.Graph().as_default(), tf.Session() as sess:
    ...
    # Define a shallow classification export and associated training ops on top
    # of VGGish.
    ...
    # Initialize all variables in the export, and then load the pre-trained
    # VGGish checkpoint.
