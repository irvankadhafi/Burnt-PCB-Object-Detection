...
def run_training(train_op,
  ...
  # Delay starting standard services to allow possible pretrained export restore.
...
def maybe_restore_pretrained_model(sess, saver_for_restore, model_dir):
  """Restores pretrained export if there is no ckpt export."""
  ...
  if not (pretrain_ckpt and pretrain_ckpt.model_checkpoint_path):
    raise ValueError(
        'Asked to restore export from %s but no checkpoint found.' % model_dir)
