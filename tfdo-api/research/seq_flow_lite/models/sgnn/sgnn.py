...
The export will be constructed in this way:
...
# Hparam collections that will be used to tune the export.
...
def fused_project(ngrams, hash_seed, buckets):
  """A wrapper to fuse project method when converting to TFLite export.
...
def keras_model(hash_seed, ngram_size, fc_size_list, hparams):
  """Compiles a keras export from projected features to labels.
  ...
  Args:
    ...
    hparams: hyper parameters for the export.

  Returns:
    A keras export that predicts the language id.
