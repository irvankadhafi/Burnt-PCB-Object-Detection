...
# A map of names to methods that help build the export.
...
def provide_groundtruth(model, labels):
  """Provides the labels to a export as groundtruth.
  ...
  Args:
    model: The detection export to provide groundtruth to.
...
def create_model_fn(detection_model_fn, configs, hparams=None, use_tpu=False,
  ...
  """Creates a export function for `Estimator`.

  Args:
    detection_model_fn: Function that returns a `DetectionModel` instance.
    configs: Dictionary of pipeline config objects.
    hparams: `HParams` object.
    use_tpu: Boolean indicating whether export should be constructed for
        use on TPU.
  ...
  def model_fn(features, labels, mode, params=None):
    """Constructs the object detection export.
    ...
    Returns:
      An `EstimatorSpec` that encapsulates the export and its serving
        configurations.
    """
    params = params or {}
...
def create_train_and_eval_specs(train_input_fn,
  ...
  Args:
    ...
    eval_on_train_data: Whether to evaluate export on training data. Default is
      False.
...
def continuous_eval_generator(estimator,
                              ...
                              name,
                              max_retries=0):
  """Perform continuous evaluation on checkpoints written to a export directory.
...
def continuous_eval(estimator,
                    ...
                    name,
                    max_retries=0):
  """Performs continuous evaluation on checkpoints written to a export directory.
