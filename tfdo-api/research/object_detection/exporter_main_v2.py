...
r"""Tool to export an object detection export for inference.

Prepares an object detection tensorflow graph for inference using export
configuration and a trained checkpoint. Outputs associated checkpoint files,
a SavedModel, and a copy of the export config.
...
specified option.
  ...
  * `image_and_boxes_tensor`: Accepts a 4-D image tensor of size
    ...
    bounding boxes. To be able to support this option, the export needs
    to implement a predict_masks_from_boxes method. See the documentation
    for DetectionFromImageAndBoxModule for details.

and the following output nodes returned by the export.postprocess(..):
...
flags.DEFINE_string('side_input_names', '',
                    'If use_side_inputs is True, this explicitly sets '
                    'the names of the side input tensors required by the export '
