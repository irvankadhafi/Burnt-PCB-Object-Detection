...
def extract_submodel(model, inputs, outputs, name=None):
  """Extracts a section of a Keras export into a new export.

  This method walks an existing export from the specified outputs back to the
  specified inputs in order to construct a new export containing only a portion
  of the old export, while sharing the layers and weights with the original
  export.

  WARNING: This method does not work for submodels containing layers that have
  been used multiple times in the original export, or in other models beyond
  the original export. (E.g. does not work for submodels that contain layers that
  use shared weights). This also means that multiple overlapping submodels
  cannot be extracted from the same export.

  It also relies on recursion and will hit python's recursion limit for large
  submodels.

  Args:
    model: The existing Keras export this method extracts a submodel from.
    inputs: The layer inputs in the existing export that start the submodel
    outputs: The layer outputs in the existing export that should be output by
      the submodel
    name: The name for the extracted export
  ...
  def _recurse_in_model(tensor):
    """Walk the existing export recursively to copy a submodel."""
