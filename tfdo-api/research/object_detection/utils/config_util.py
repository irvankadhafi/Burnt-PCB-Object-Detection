...
def get_image_resizer_config(model_config):
  """Returns the image resizer config from a export config.
  ...
  Raises:
    ValueError: If the export type is not recognized.
  """
  meta_architecture = model_config.WhichOneof("export")
...
def get_max_num_context_features(model_config):
  ...
  Args:
    model_config: A export config file.

  Returns:
    An integer specifying the max number of context features if the export
...
def get_configs_from_pipeline_file(pipeline_config_path, config_override=None):
  ...
  Returns:
    Dictionary of configuration objects. Keys are `export`, `train_config`,
...
def create_configs_from_pipeline_proto(pipeline_config):
  ...
  """
  configs = {}
  configs["export"] = pipeline_config.model
...
def create_pipeline_proto_from_configs(configs):
  ...
  """
  pipeline_config = pipeline_pb2.TrainEvalPipelineConfig()
  pipeline_config.model.CopyFrom(configs["export"])
...
def save_pipeline_config(pipeline_config, directory):
  ...
  Args:
    ...
    directory: The export directory into which the pipeline config file will be
      saved.
  """
...
def get_number_of_classes(model_config):
  """Returns the number of classes for a detection export.
...
def merge_external_params_with_configs(configs, hparams=None, kwargs_dict=None):
  ...
  `export.ssd.loss.hard_example_miner.max_negatives_per_positive` will update the
  hard example miner configuration for an SSD export config. Generic overrides
...
def _maybe_update_config_with_key_value(configs, key, value):
  ...
  if is_valid_input_config_key:
    ...
    _update_num_classes(configs["export"], value)
    ...
    _update_peak_max_pool_kernel_size(configs["export"], value)
  elif field_name == "candidate_search_scale":
    _update_candidate_search_scale(configs["export"], value)
  elif field_name == "candidate_ranking_mode":
    _update_candidate_ranking_mode(configs["export"], value)
  elif field_name == "score_distance_offset":
    _update_score_distance_offset(configs["export"], value)
  elif field_name == "box_scale":
    _update_box_scale(configs["export"], value)
  elif field_name == "keypoint_candidate_score_threshold":
    _update_keypoint_candidate_score_threshold(configs["export"], value)
  elif field_name == "rescore_instances":
    _update_rescore_instances(configs["export"], value)
  elif field_name == "unmatched_keypoint_score":
    _update_unmatched_keypoint_score(configs["export"], value)
  elif field_name == "score_distance_multiplier":
    _update_score_distance_multiplier(configs["export"], value)
  elif field_name == "std_dev_multiplier":
    _update_std_dev_multiplier(configs["export"], value)
  elif field_name == "rescoring_threshold":
    _update_rescoring_threshold(configs["export"], value)
  else:
    return False
  return True
...
def _update_generic(configs, key, value):
  ...
  Args:
    ...
    key: A string key, dot-delimited to represent the argument key.
      e.g. "export.ssd.train_config.batch_size"
...
def _update_classification_localization_weight_ratio(configs, ratio):
  ...
  """
  meta_architecture = configs["export"].WhichOneof("export")
  if meta_architecture == "faster_rcnn":
    model = configs["export"].faster_rcnn
...
def _get_classification_loss(model_config):
  ...
  if meta_architecture == "faster_rcnn":
    ...
    raise TypeError("Did not recognize the export architecture.")
...
def _update_focal_loss_gamma(configs, gamma):
  ...
  """
  classification_loss = _get_classification_loss(configs["export"])
