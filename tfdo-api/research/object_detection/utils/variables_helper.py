    ...
    if variable_name in ckpt_vars_to_shape_map:
      if ckpt_vars_to_shape_map[variable_name] == variable.shape.as_list():
        ...
        logging.warning('Variable [%s] is available in checkpoint, but has an '
                        'incompatible shape with export variable. Checkpoint '
                        'shape: [%s], export variable shape: [%s]. This '
...
def get_global_variables_safely():
  ...
  If executing eagerly, use a Keras export's .variables property instead.
  ...
  with tf.init_scope():
    if tf.executing_eagerly():
      raise ValueError("Global variables collection is not tracked when "
                       "executing eagerly. Use a Keras export's `.variables` "
                       "attribute instead.")
...
def ensure_checkpoint_supported(checkpoint_path, checkpoint_type, model_dir):
  ...
  Args:
    ...
    model_dir: The export directory to store intermediate training checkpoints.
