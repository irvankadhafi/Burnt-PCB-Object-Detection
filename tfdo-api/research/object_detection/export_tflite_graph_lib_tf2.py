...
class SSDModule(tf.Module):
  ...
  def __init__(self, pipeline_config, detection_model, max_detections,
    ...
    Args:
      ...
      detection_model: The detection export to use for inference.
      max_detections: Max detections desired from the TFLite export.
      use_regular_nms: If True, TFLite export uses the (slower) multi-class NMS.
    """
  ...
  def input_shape(self):
    """Returns shape of TFLite export input."""
  ...
  def inference_fn(self, image):
    ...
    NOTE: The Args & Returns sections below indicate the TFLite export signature,
...
class CenterNetModule(tf.Module):
  ...
  The exported CenterNet export includes the preprocessing and postprocessing
...
def export_tflite_model(pipeline_config, trained_checkpoint_dir,
  ...
  NOTE: Only supports SSD meta-architectures for now, and the output export will
  ...
  Args:
    ...
    max_detections: Max detections desired from the TFLite export.
    use_regular_nms: If True, TFLite export uses the (slower) multi-class NMS.
      Note that this argument is only used by the SSD export.
    include_keypoints: Decides whether to also output the keypoint predictions.
      Note that this argument is only used by the CenterNet export.
    label_map_path: Path to the label map which is used by CenterNet keypoint
  ...
  # Build the underlying export using pipeline config.
  # TODO(b/162842801): Add support for other architectures.
  if pipeline_config.model.WhichOneof('export') == 'ssd':
  ...
  elif pipeline_config.model.WhichOneof('export') == 'center_net':
    ...
    raise ValueError('Only ssd or center_net models are supported in tflite. '
                     'Found {} in config'.format(
                         pipeline_config.model.WhichOneof('export')))
  ...
  # be constructed; only after this can we save the saved export.
