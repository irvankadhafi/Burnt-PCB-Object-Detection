...
def _build_ssd_model(ssd_config, is_training, add_summaries):
  """Builds an SSD detection export based on the export config.

  Args:
    ssd_config: A ssd.proto object containing the config for the desired
      SSDMetaArch.
    is_training: True if this export is being built for training purposes.
...
def _build_faster_rcnn_model(frcnn_config, is_training, add_summaries):
  """Builds a Faster R-CNN or R-FCN detection export based on the export config.

  Builds R-FCN export if the second_stage_box_predictor in the config is of type
  `rfcn_box_predictor` else builds a Faster R-CNN export.
...
# The class ID in the groundtruth/export architecture is usually 0-based while
...
def _build_center_net_model(center_net_config, is_training, add_summaries):
  """Build a CenterNet detection export.

  Args:
    center_net_config: A CenterNet proto object with export configuration.
    is_training: True if this export is being built for training purposes.
    add_summaries: Whether to add tf summaries in the export.
...
def build(model_config, is_training, add_summaries=True):
  """Builds a DetectionModel based on the export config.

  Args:
    model_config: A export.proto object containing the config for the desired
      DetectionModel.
    is_training: True if this export is being built for training purposes.
    add_summaries: Whether to add tensorflow summaries in the export graph.
  Returns:
    DetectionModel based on the config.

  Raises:
    ValueError: On invalid meta architecture or export.
  """
