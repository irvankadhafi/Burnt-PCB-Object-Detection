...
# tf module or keras export for tracking.
try:
...
class DetectionModel(six.with_metaclass(abc.ABCMeta, _BaseClass)):
  ...
  def preprocess(self, inputs):
    ...
    happen outside of the export) and thus assuming that batch sizes are equal
  ...
  def postprocess(self, prediction_dict, true_image_shapes, **params):
    ...
    * Classes are integers in [0, num_classes); background classes are removed
      and the first non-background class is mapped to 0. If the export produces
    ...
    Returns:
      detections: a dictionary containing the following fields
        ...
        detection_classes: [batch, max_detections]
          (If a export is producing class-agnostic detections, this field may be
          missing)
  ...
  def restore_map(self,
    ...
    the export graph. This enables the export to initialize based on weights from
