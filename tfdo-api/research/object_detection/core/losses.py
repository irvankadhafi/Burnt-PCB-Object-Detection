...
class BootstrappedSigmoidClassificationLoss(Loss):
  ...
  This loss uses a convex combination of training labels and the current export's
  predictions as training targets in the classification loss. The idea is that
  as the export improves over time, its predictions can be trusted more and we
  ...
  other stimuli predicted to have the same label by the export.

  In "soft" bootstrapping, we use all predicted class probabilities, whereas in
  "hard" bootstrapping, we use the single class favored by the export.
