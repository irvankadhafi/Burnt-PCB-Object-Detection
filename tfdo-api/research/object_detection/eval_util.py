...
def _run_checkpoint_once(tensor_dict,
  ...
  Args:
    ...
    variables_to_restore: None, or a dictionary mapping variable names found in
      a checkpoint to export variables. The dictionary would normally be
  ...
  with slim.queues.QueueRunners(sess):
    try:
      ...
      if process_metrics_fn and checkpoint_file:
        m = re.search(r'export.ckpt-(\d+)$', checkpoint_file)
        if not m:
...
def repeated_checkpoint_run(tensor_dict,
  ...
  while True:
    ...
    if not model_path:
      tf.logging.info('No export found in %s. Will try again in %d seconds',
