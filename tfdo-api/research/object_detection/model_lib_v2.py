...
# ==============================================================================
r"""Constructs export, inputs, and training environment."""
...
def _compute_losses_and_predictions_dicts(
  ...
  """Computes the losses dict and predictions dict for a export on inputs.
  ...
  Args:
    ...
    add_regularization_loss: Whether or not to include the export's
...
def _ensure_model_is_built(model, input_dataset, unpad_groundtruth_tensors):
  """Ensures that export variables are all built, by running on a dummy input.

  Args:
    model: A DetectionModel to be built.
    input_dataset: The tf.data Dataset the export is being trained on. Needed to
...
def eager_train_step(detection_model,
  ...
  This method computes the loss for the export on a single training batch,
  while tracking the gradients with a gradient tape. It then updates the
  export variables with the optimizer, clipping the gradients if
...
def load_fine_tune_checkpoint(model, checkpoint_path, checkpoint_type,
  ...
  To make sure the export variables are all built, this method first executes
  the export by computing a dummy loss. (Models might not have built their
  variables before their first execution)

  It then loads an object-based classification or detection checkpoint.

  This method updates the export in-place and does not return a value.
  ...
  Args:
    ...
    run_model_on_dummy_input: Whether to run the export on a dummy input in order
      to ensure that all export variables have been built successfully before
      loading the fine_tune_checkpoint.
    input_dataset: The tf.data Dataset the export is being trained on. Needed
...
def train_loop(
    ...
    **kwargs):
  """Trains a export using eager + functions.

  This method:
    1. Processes the pipeline configs
    2. (Optionally) saves the as-run config
    3. Builds the export & optimizer
    ...
    6. Loops over the train data, executing distributed training steps inside
       tf.functions.
    7. Checkpoints the export every `checkpoint_every_n` training steps.
  ...
  Args:
    ...
    checkpoint_max_to_keep:
      int, the number of most recent checkpoints to keep in the export directory.
    record_summaries: Boolean, whether or not to record summaries defined by
      the export or the training pipeline. This does not impact the summaries
  ...
  # Build the export, optimizer, and training input
  strategy = tf.compat.v2.distribute.get_strategy()
  with strategy.scope():
    ...
    # export and all its variables have been properly constructed. Specifically,
    # this is currently necessary prior to (potentially) creating shadow copies
    # of the export variables for the EMA optimizer.
  ...
  with summary_writer.as_default():
    with strategy.scope():
      with tf.compat.v2.summary.record_if(
        ...
        # manager_dir does not point to the export directory when we are running
        # in a worker.
...
def prepare_eval_dict(detections, groundtruth, features):
  ...
  Takes in `detections` from the export, `groundtruth` and `features` returned
  from the eval tf.data.dataset and creates a dictionary of tensors suitable
  for detection eval modules.

  Args:
    detections: A dictionary of tensors returned by `export.postprocess`.
...
def eager_eval_loop(
  ...
  """Evaluate the export eagerly on the evaluation dataset.
  ...
  Args:
    ...
    use_tpu: Whether a TPU is being used to execute the export for evaluation.
    postprocess_on_cpu: Whether export postprocessing should happen on
      the CPU when using a TPU to execute the export.
    global_step: A variable containing the training step this export was trained
...
def eval_continuously(
    ...
    save_final_config=False,
    **kwargs):
  """Run continuous evaluation of a detection export eagerly.

  This method builds the export, and continously restores it from the most
  ...
  Args:
    ...
    save_final_config: Whether to save the pipeline config file to the export
      directory.
    **kwargs: Additional keyword arguments for configuration override.
  """
  get_configs_from_pipeline_file = MODEL_BUILD_UTIL_MAP[
