...
Tests comparing this export to the existing Tensorflow export can be
found at
...
def _obtain_input_shape(
    ...
    data_format,
    require_flatten):
  """Internal utility to compute/validate an ImageNet export's input shape.

  Args:
      input_shape: either None (will return the default network input shape),
          or a user-provided shape to be validated.
      default_size: default input width/height for the export.
      min_size: minimum input width/height accepted by the export.
      data_format: image data format to use.
      require_flatten: whether the export is expected to
  ...
  if input_shape and len(input_shape) == 3:
    if data_format == 'channels_first':
      if input_shape[0] not in {1, 3}:
        warnings.warn(
            'This export usually expects 1 or 3 input channels. '
...
def mobilenet_v2(input_shape=None,
  ...
  To load a MobileNetV2 export via `load_model`, import the custom
  objects `relu6` and pass them to the `custom_objects` parameter.
  E.g.
  export = load_model('mobilenet.h5', custom_objects={
                     'relu6': mobilenet.relu6})

  Args:
    input_shape: optional shape tuple, to be specified if you would
      like to use a export with an input img resolution that is not
      (224, 224, 3).
  ...
  if K.image_data_format() != 'channels_last':
    warnings.warn('The MobileNet family of models is only available '
                  ...
                  'The export being returned right now will expect inputs '
  ...
  # Ensure that the export takes into account
