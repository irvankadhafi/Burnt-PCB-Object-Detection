...
(e.g., predict, postprocess, loss) can be used as if the export consisted
...
stage of the export), and `_extract_box_classifier_features` (the second stage of
the export). Optionally, the `restore_fn` method can be overridden.  See tests
for an example.
  ...
  def __init__(self,
    ...
    Args:
      ...
      number_of_stages:  Valid values are {1, 2}. If 1 will only construct the
        Region Proposal Network (RPN) part of the export.
      ...
      second_stage_target_assigner: Target assigner to use for second stage of
        R-FCN. If the export is configured with multiple prediction heads, this
