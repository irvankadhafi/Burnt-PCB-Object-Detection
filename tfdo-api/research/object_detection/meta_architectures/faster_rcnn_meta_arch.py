...
if the export consisted only of the RPN, returning class agnostic proposals
...
`_extract_proposal_features` (the first stage of the export), and
`_extract_box_classifier_features` (the second stage of the export). Optionally,
...
directly as, e.g., a export constructor).
...
Following the API (see export.DetectionModel definition), our outputs after
...
class FasterRCNNKerasFeatureExtractor(object):
  ...
  def get_proposal_feature_extractor_model(self, name):
    """Get export that extracts first stage RPN features, to be overridden."""
    pass

  @abc.abstractmethod
  def get_box_classifier_feature_extractor_model(self, name):
    """Get export that extracts second stage box classifier features."""
    pass
...
class FasterRCNNMetaArch(model.DetectionModel):
  ...
  def __init__(self,
    ...
    Args:
      ...
      number_of_stages:  An integer values taking values in {1, 2, 3}. If
        ...
        part of the export. If 2, the function will perform box refinement and
        ...
        reduced to 2 since all the export heads are trained in parallel in second
      ...
      first_stage_box_predictor_arg_scope_fn: Either a
        ...
        RPN box predictor will be a Keras export. If it is a function to
      ...
      second_stage_target_assigner: Target assigner to use for second stage of
        Faster R-CNN. If the export is configured with multiple prediction heads,
      ...
      resize_masks: Indicates whether the masks presend in the groundtruth
        should be resized in the export with `image_resizer_fn`
    ...
    if isinstance(feature_extractor, FasterRCNNKerasFeatureExtractor):
      ...
      # to avoid creating the variables when a export is built just for data
    ...
    Returns:
      prediction_dict: a dictionary holding "raw" prediction tensors:
        ...
        6) anchors: A 2-D tensor of shape [num_anchors, 4] representing anchors
          ...
          `num_anchors` can differ depending on whether the export is created in
  ...
  def postprocess(self, prediction_dict, true_image_shapes):
    ...
    from the full two-stage export and are padded to self._max_detections.
  ...
  def _postprocess_rpn(self,
    ...
    Args:
      ...
      anchors: A 2-D tensor of shape [num_anchors, 4] representing anchors
        ...
        on whether the export is created in training or inference mode.
  ...
  def _format_groundtruth_data(self, image_shapes):
    ...
    In order to be consistent with the export.DetectionModel interface,
  ...
  def regularization_losses(self):
    """Returns a list of regularization losses for this export.

    Returns a list of regularization losses for this export that the estimator
  ...
  def updates(self):
    """Returns a list of update operators for this export.

    Returns a list of update operators for this export that must be executed at
    ...
    # executed and don't depend on any export inputs in the graph.
