...
class InputFnTest(test_case.TestCase, parameterized.TestCase):
  ...
  def test_error_with_bad_train_config(self):
    ...
    configs = _get_configs_for_model('ssd_inception_v2_pets')
    configs['export'].ssd.num_classes = 37
    train_input_fn = inputs.create_train_input_fn(
  ...
  def test_error_with_bad_train_input_config(self):
    ...
    train_input_fn = inputs.create_train_input_fn(
        train_config=configs['train_config'],
        train_input_config=configs['export'],  # Expecting `InputReader`.
        model_config=configs['export'])
    with self.assertRaises(TypeError):
      train_input_fn()

  def test_error_with_bad_train_model_config(self):
    """Tests that a TypeError is raised with improper train export config."""
    configs = _get_configs_for_model('ssd_inception_v2_pets')
    configs['export'].ssd.num_classes = 37
    train_input_fn = inputs.create_train_input_fn(
        train_config=configs['train_config'],
  ...
  def test_error_with_bad_eval_input_config(self):
    ...
    eval_input_fn = inputs.create_eval_input_fn(
        eval_config=configs['eval_config'],
        eval_input_config=configs['export'],  # Expecting `InputReader`.
        model_config=configs['export'])
    with self.assertRaises(TypeError):
      eval_input_fn()

  def test_error_with_bad_eval_model_config(self):
    """Tests that a TypeError is raised with improper eval export config."""
    configs = _get_configs_for_model('ssd_inception_v2_pets')
    configs['export'].ssd.num_classes = 37
    eval_input_fn = inputs.create_eval_input_fn(
        eval_config=configs['eval_config'],
