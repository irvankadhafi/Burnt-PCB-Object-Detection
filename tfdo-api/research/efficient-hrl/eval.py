...
def evaluate(checkpoint_dir,
  ...
  """Loads and repeatedly evaluates a checkpointed export at a set interval.
  ...
  Args:
    ...
    eval_model_rollout: Evaluate export rollout.
  ...
  if checkpoint_path is not None:
    ...
    model_files = tf.gfile.Glob(
        os.path.join(checkpoint_dir, 'export.ckpt-*.index'))
    tf.logging.info('Found %s policies at %s', len(model_files), checkpoint_dir)
    model_files = {
        int(f.split('export.ckpt-', 1)[1].split('.', 1)[0]):
