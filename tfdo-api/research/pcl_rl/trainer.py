...
flags.DEFINE_string('save_dir', '', 'directory to save export to')
flags.DEFINE_string('load_path', '', 'path of saved export to load (if none in save_dir)')
  ...
  def run(self):
    ...
    for step in xrange(1 + self.num_steps):
      ...
      if is_chief and step % self.validation_frequency == 0:
        logging.info('at training step %d, export step %d: '
    ...
    if is_chief and sv is not None:
      logging.info('saving final export to %s', sv.save_path)
