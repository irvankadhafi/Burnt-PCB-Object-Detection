...
class Config(object):
  """Stores everything needed to train a export."""

  def __init__(self, **kwargs):
    # general
    self.data_dir = './data'  # top directory for data (corpora, models, etc.)
    self.model_name = 'default_model'  # name identifying the current export

    # mode
    self.mode = 'train'  # either "train" or "eval"
    self.task_names = ['chunk']  # list of tasks this export will learn
                                 # more than one trains a multi-task export
    ...
    # EMA
    self.ema_decay = 0.998  # EMA coefficient for averaged export weights
    self.ema_test = True  # whether to use EMA weights at test time
    self.ema_teacher = False  # whether to use EMA weights for the teacher export
    ...
    self.save_model_every = 1000  # how often to checkpoint the export
