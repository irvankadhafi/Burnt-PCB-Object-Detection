...
# ==============================================================================
"""ResNet50 backbone used in DELF export.
...
class ResNet50(tf.keras.Model):
  ...
  Args:
    ...
    name: Prefix applied to names of variables created in the export.
    include_top: whether to include the fully-connected layer at the top of the
      network.
    pooling: Optional pooling mode for feature extraction when `include_top` is
      False. 'None' means that the output of the export will be the 4D tensor
      ...
      thus the output of the export will be a 2D tensor. 'max' means that global
  ...
  def build_call(self, inputs, training=True, intermediates_dict=None):
    """Building the ResNet50 export.

    Args:
      inputs: Images to compute features for.
      training: Whether export is in training phase.
  ...
  def restore_weights(self, filepath):
    ...
    This function loads a .h5 file from the filepath with saved export weights
    and assigns them to the export.
      ...
      for layer in self.layers:
        if hasattr(layer, 'layers'):
          for inlayer in layer.layers:
            # Make sure the weights are in the saved export, and that we are in
            # the innermost layer.
