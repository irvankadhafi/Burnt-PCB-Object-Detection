...
# TODO(sguada) describe the high-level components of export deployment.
# E.g. "each export deployment is composed of several parts: a DeploymentConfig,
# which captures A, B and C, an input_fn which loads data.. etc

To easily train a export on multiple GPUs or across multiple machines this
...
Usage:
  ...
  # Define the export including the loss.
...
DeploymentConfig parameters:
  * num_clones: Number of export clones to deploy in each replica.
  * clone_on_cpu: True if clones should be placed on CPU.
  * replica_id: Integer.  Index of the replica for which the export is
...
def create_clones(config, model_fn, args=None, kwargs=None):
  ...
  export clones as `model_fn(*args, **kwargs)`.

  If `config` specifies deployment on multiple replicas then the default
  tensorflow device is set appropriatly for each call to `model_fn` and for the
  slim variable creation functions: export and global variables will be created
...
def deploy(config,
  ...
  """Deploys a Slim-constructed export across multiple clones.
  ...
  the deployed export is configured for training with that optimizer.
  ...
  Args:
    ...
    args: Optional list of arguments to pass to `model_fn`.
    kwargs: Optional list of keyword arguments to pass to `model_fn`.
    optimizer: Optional `Optimizer` object.  If passed the export is deployed
...
class DeploymentConfig(object):
  """Configuration for deploying a export with `deploy()`.

  You can pass an instance of this class to `deploy()` to specify exactly
  how to deploy the export to build.  If you do not pass one, an instance built
  ...
  def __init__(self,
    ...
    The config describes how to deploy a export across multiple clones and
    replicas.  The export will be replicated `num_clones` times in each replica.
    If `clone_on_cpu` is True, each clone will placed on CPU.

    If `num_replicas` is 1, the export is deployed via a single process.  In that
