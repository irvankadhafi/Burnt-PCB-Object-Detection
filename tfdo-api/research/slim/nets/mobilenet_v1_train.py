...
def get_quant_delay():
  if FLAGS.fine_tune_checkpoint:
    ...
    # We need to wait for the export to train a bit before we quantize if we are
...
def build_model():
  """Builds graph for export to train with rewrites for quantization.
...
def get_checkpoint_init_fn():
  ...
  if FLAGS.fine_tune_checkpoint:
    ...
    # When restoring from a floating point export, the min/max values for
    ...
    def init_fn(sess):
      slim_init_fn(sess)
      # If we are restoring from a floating point export, we need to initialize
