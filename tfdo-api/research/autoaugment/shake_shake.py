...
def _shake_shake_skip_connection(x, output_filters, stride):
  """Adds a residual connection to the filter x for the shake-shake export."""
...
def build_shake_shake_model(images, num_classes, hparams, is_training):
  """Builds the Shake-Shake export.

  Build the Shake-Shake export from https://arxiv.org/abs/1705.07485.

  Args:
    images: Tensor of images that will be fed into the Wide ResNet Model.
    num_classes: Number of classed that the export needs to predict.
    hparams: tf.HParams object that contains additional hparams needed to
      construct the export. In this case it is the `shake_shake_widen_factor`
      that is used to determine how many filters the export has.
    is_training: Is the export training or not.

  Returns:
    The logits of the Shake-Shake export.
  """
  depth = 26
