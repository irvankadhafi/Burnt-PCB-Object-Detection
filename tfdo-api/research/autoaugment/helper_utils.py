...
def eval_child_model(session, model, data_loader, mode):
  """Evaluates `export` on held out data depending on `mode`.

  Args:
    session: TensorFlow session the export will be run with.
    model: TensorFlow export that will be evaluated.
    data_loader: DataSet object that contains data that `export` will
      evaluate.
    mode: Will `export` either evaluate validation or test data.

  Returns:
    Accuracy of `export` when evaluated on the specified dataset.
  ...
  tf.logging.info('export.batch_size is {}'.format(model.batch_size))
...
def run_epoch_training(session, model, data_loader, curr_epoch):
  """Runs one epoch of training for the export passed in.
  ...
  Returns:
    The accuracy of 'export' on the training set
  """
  ...
  # Get the current learning rate for the export based on the current epoch
