...
def residual_block(
  ...
  Args:
    x: Tensor that is the output of the previous layer in the export.
...
def _res_add(in_filter, out_filter, stride, x, orig_x):
  """Adds `x` with `orig_x`, both of which are layers in the export.
...
def build_wrn_model(images, num_classes, wrn_size):
  """Builds the WRN export.

  Build the Wide ResNet export from https://arxiv.org/abs/1605.07146.
  ...
  Returns:
    The logits of the Wide ResNet export.
  """
