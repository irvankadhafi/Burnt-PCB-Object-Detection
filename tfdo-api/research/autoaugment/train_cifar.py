...
def setup_arg_scopes(is_training):
  """Sets up the argscopes that will be used when building an image export.

  Args:
    is_training: Is the export training or not.

  Returns:
    Arg scopes to be put around the export being constructed.
  """
...
def build_model(inputs, num_classes, is_training, hparams):
  """Constructs the vision export being trained/evaled.

  Args:
    inputs: input features/images being fed to the image export build built.
    num_classes: number of output classes being predicted.
    is_training: is the export training or not.
    hparams: additional hyperparameters associated with the image export.

  Returns:
    The logits of the image export.
  """
...
class CifarModel(object):
  """Builds an image export for Cifar10/Cifar100."""

  def __init__(self, hparams):
    self.hparams = hparams

  def build(self, mode):
    """Construct the cifar export."""
  ...
  def _setup_misc(self, mode):
    """Sets up miscellaneous in the cifar export constructor."""
  ...
  def _setup_images_and_labels(self):
    """Sets up image and label placeholders for the cifar export."""
  ...
  def _build_graph(self, images, labels, mode):
    """Constructs the TF graph for the cifar export.
    ...
    # Setup checkpointing for this child export
  ...
  def _build_train_op(self):
    """Builds the train op for the cifar export."""
...
class CifarModelTrainer(object):
  ...
  def extract_model_spec(self):
    ...
    if checkpoint_path is not None:
      ...
      tf.logging.info('Loaded child export checkpoint from %s',
                      checkpoint_path)
    else:
      self.save_model(step=0)

  def eval_child_model(self, model, data_loader, mode):
    """Evaluate the child export.

    Args:
      model: image export that will be evaluated.
      data_loader: dataset object to extract eval data from.
      mode: will the export be evalled on train, val or test.

    Returns:
      Accuracy of the export on the specified dataset.
    """
    tf.logging.info('Evaluating child export in mode %s', mode)
    while True:
      try:
        with self._new_session(model):
          accuracy = helper_utils.eval_child_model(
              ...
              data_loader,
              mode)
          tf.logging.info('Eval child export accuracy: {}'.format(accuracy))
  ...
  def _new_session(self, m):
    """Creates a new session for export m."""
    # Create a new session for this export, initialize
  ...
  def _build_models(self):
    ...
    # Determine if we should build the train and eval export. When using
    # distributed training we only want to build one or the other and not both.
    with tf.variable_scope('export', use_resource=False):
      m = CifarModel(self.hparams)
      m.build('train')
      self._num_trainable_params = m.num_trainable_params
      self._saver = m.saver
    with tf.variable_scope('export', reuse=True, use_resource=False):
      meval = CifarModel(self.hparams)
      meval.build('eval')
    return m, meval

  def _calc_starting_epoch(self, m):
    """Calculates the starting epoch for export m based on global step."""
    hparams = self.hparams
  ...
  def _run_training_loop(self, m, curr_epoch):
    """Trains the cifar export `m` for one epoch."""
    ...
    while True:
      try:
        with self._new_session(m):
          ...
          tf.logging.info('Saving export after epoch')
  ...
  def run_model(self):
    """Trains and evalutes the image export."""
    hparams = self.hparams
