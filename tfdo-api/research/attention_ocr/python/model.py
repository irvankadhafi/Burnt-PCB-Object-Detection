...
# ==============================================================================
"""Functions to build the Attention OCR export.

Usage example:
  ocr_model = export.Model(num_char_classes, seq_length, num_of_views)

  data = ... # create namedtuple InputEndpoints
  endpoints = export.create_base(data.images, data.labels_one_hot)
  # endpoints.predicted_chars is a tensor with predicted character codes.
  total_loss = export.create_loss(data, endpoints)
"""
import sys
import collections
import logging
...
class Model(object):
  ...
  def conv_tower_fn(self, images, is_training=True, reuse=None):
    """Computes convolutional features using the InceptionV3 export.
  ...
  def create_base(self,
    ...
    Args:
      ...
      labels_one_hot: Optional (can be None) one-hot encoding for ground truth
        labels. If provided the function will create a export for training.
  ...
  def create_loss(self, data, endpoints):
    """Creates all losses required to train the export.
    ...
    # as part of GetTotalLoss. We need to use total loss because export may have
  ...
  def create_summaries(self, data, endpoints, charset, is_training):
    """Creates all summaries for the export.
