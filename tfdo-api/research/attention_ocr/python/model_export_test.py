...
_CHECKPOINT = 'export.ckpt-399731'
_CHECKPOINT_URL = (
...
def _create_tf_example_string(image):
  """Create a serialized tf.Example proto for feeding the export."""
...
class AttentionOcrExportTest(tf.test.TestCase):
  ...
  def create_input_feed(self, graph_def, serving):
    """Returns the input feed for the export.

    Creates random images, according to the size specified by dataset_name,
    format it in the correct way depending on whether the export was exported
    for serving, and return the correctly keyed feed_dict for inference.

    Args:
      graph_def: Graph definition of the loaded export.
      serving: Whether the export was exported for Serving.

    Returns:
      The feed_dict suitable for export inference.
    """
  ...
  def verify_export_load_and_inference(self, export_for_serving=False):
    """Verify exported export can be loaded and inference can run successfully.

    This function will load the exported export in self.export_dir, then create
    some fake images according to the specification of FLAGS.dataset_name.
    It then feeds the input through the export, and verify the minimal set of
    ...
    Note: Model and dataset creation in the underlying library depends on the
          following commandline flags:
            FLAGS.dataset_name
    Args:
      export_for_serving: True if the export was exported for Serving. This
        affects how input is fed into the export.
    """
